 ...
 *恢复出厂设置。此接口应由路由器或终端设备调用。如果它不是出厂新设备，它将在出厂新重置之前广播离开命令。
 * @param       None
 *
 * @return      None
 *
 ...
 * 					a Leave Command before factory new reset.
 *将设备重置为出厂新设备。
此接口可由协调器、路由器或终端设备调用。如果协调员：
它将擦除所有NV信息并重置所有层设置。如果是路由器或终端设备：
如果它不是出厂时的新设备，它将进行广播
a出厂重置前的离开命令。
 * @param       None
 *
...
/***********************************************************************//**
 * @brief       Get MAC association permit status (only for ZC ZR) 获取MAC关联许可状态（仅适用于ZC ZR）
 *
 * @param		None
 *
 * @return      Status
 ...
 * 				Only for Coordinator and Router.
 *向本地设备执行许可加入请求。仅适用于协调员和路由器。   lsh
 * @param[in]   permitDuration - The time allowed for new device to join (in seconds). 允许新设备加入的时间（秒）。
 *
 * @return      Status
 *
 **************************************************************************/
