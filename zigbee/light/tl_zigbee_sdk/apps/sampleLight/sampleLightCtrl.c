...
void pwmSetDuty(u8 ch, u16 dutycycle)
{
    ...
    printf("[%s:%d] cmp_tick:%d,PMW_MAX_TICK:%d\r\n", __FUNCTION__, __LINE__, cmp_tick, PMW_MAX_TICK);
    drv_pwm_cfg(ch, (u16)cmp_tick, PMW_MAX_TICK); // PMW_MAX_TICK:48000  ��TICK����  cmp_tick��48000һ�������иߵ�ƽtick��
#endif
}
...
void hwLight_onOffUpdate(u8 onOff)
{
    if (onOff)
    {
        ...
        drv_pwm_start(Y_LIGHT_PWM_CHANNEL);
        drv_pwm_start(W_LIGHT_PWM_CHANNEL);
#else
#if COLOR_CCT_SUPPORT
    ...
    else
    {
        ...
        drv_pwm_stop(Y_LIGHT_PWM_CHANNEL);
        drv_pwm_stop(W_LIGHT_PWM_CHANNEL);
#else
#if COLOR_CCT_SUPPORT
...
void hwLight_levelUpdate(u8 level)
{
    ...
    pwmSetDuty(COOL_LIGHT_PWM_CHANNEL, gammaCorrectLevel * PWM_FULL_DUTYCYCLE); // pwm设置占空比
#endif
}
 ...
 * @brief
 *
 * @param   hue			-	hue attribute value    ɫ������ֵ
 * 			saturation	-	saturation attribute value ���Ͷ�����ֵ
 * 			level		-	level attribute value   �ȼ�����ֵ
 *
 * @return  None
 */
void hwLight_colorUpdate_HSV2RGB(u8 hue, u8 saturation, u8 level)
{
    ...
    pwmSetDuty(PWM_Y_CHANNEL, gammaCorrectB * PWM_FULL_DUTYCYCLE);
    pwmSetDuty(PWM_W_CHANNEL, gammaCorrectB * PWM_FULL_DUTYCYCLE);
#endif
}
