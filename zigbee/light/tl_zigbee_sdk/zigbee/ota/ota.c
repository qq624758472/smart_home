...
void ota_saveUpdateInfo2NV(void *arg)
{
    ...
    if ((++cnt >= FLASH_WRITE_COUNT_GET(g_otaCtx.downloadImageSize) || (zcl_attr_imageUpgradeStatus == IMAGE_UPGRADE_STATUS_DOWNLOAD_COMPLETE)))
    {
        cnt = 0;
    ...
    for (u8 i = 0; i < len; i++)
    {
        ...
        case OTA_FLAG_IMAGE_ELEMENT:
            if ((otaClientInfo.otaElementTag == OTA_UPGRADE_IMAGE_TAG_ID) || (otaClientInfo.otaElementTag == OTA_UPGRADE_IMAGE_AES_TAG_ID))
            {
                ...
                if ((otaClientInfo.otaElementPos < FLASH_TLNK_FLAG_OFFSET + 1) && ((otaClientInfo.otaElementPos + dataSize) >= FLASH_TLNK_FLAG_OFFSET + 1))
                {
...
static status_t ota_queryNextImageReqHandler(zclIncomingAddrInfo_t *pAddrInfo, ota_queryNextImageReq_t *pQueryNextImageReq)
{
    ...
    if ((otaServerBinInfo.manufacturerCode == pQueryNextImageReq->manuCode) && (otaServerBinInfo.imageType == pQueryNextImageReq->imageType) && (otaServerBinInfo.fileVer > pQueryNextImageReq->curFileVer))
    {
...
{
    if ((otaServerBinInfo.manufacturerCode != pImageBlockReq->manuCode) || (pImageBlockReq->fileVer != otaServerBinInfo.fileVer) || (otaServerBinInfo.imageType != pImageBlockReq->imageType))
    {
...
static status_t ota_imageNotifyHandler(zclIncomingAddrInfo_t *pAddrInfo, ota_imageNotify_t *pImageNotify)
{
    ...
    {
        if ((pImageNotify->payloadType >= IMAGE_NOTIFY_QUERT_JITTER_MFG) && (pImageNotify->manuCode != g_otaCtx.pOtaPreamble->manufacturerCode))
        {
            return ZCL_STA_SUCCESS;
        }
        if ((pImageNotify->payloadType >= IMAGE_NOTIFY_QUERY_JITTER_MFG_TYPE) && (pImageNotify->imageType != g_otaCtx.pOtaPreamble->imageType))
        {
            return ZCL_STA_SUCCESS;
        }
        if ((pImageNotify->payloadType >= IMAGE_NOTIFY_QUERY_JITTER_MFG_TYPE_VER) && (pImageNotify->newFileVer == zcl_attr_currFileVer))
        {
            return ZCL_STA_SUCCESS;
        }
...
static status_t ota_queryNextImageRspHandler(zclIncomingAddrInfo_t *pAddrInfo, ota_queryNextImageRsp_t *pQueryNextImageRsp)
{
    ...
    if ((pQueryNextImageRsp->st == ZCL_STA_SUCCESS) && (pQueryNextImageRsp->imageType == g_otaCtx.pOtaPreamble->imageType) && (pQueryNextImageRsp->manuCode == g_otaCtx.pOtaPreamble->manufacturerCode))
    {
...
static status_t ota_imageBlockRspHandler(zclIncomingAddrInfo_t *pAddrInfo, ota_imageBlockRsp_t *pImageBlockRsp)
{
    ...
    {
        if ((pImageBlockRsp->rsp.success.imageType != g_otaCtx.pOtaPreamble->imageType) || (pImageBlockRsp->rsp.success.manuCode != g_otaCtx.pOtaPreamble->manufacturerCode) || (pImageBlockRsp->rsp.success.fileVer != zcl_attr_downloadFileVer))
        {
...
{
    if ((zcl_attr_imageUpgradeStatus == IMAGE_UPGRADE_STATUS_DOWNLOAD_COMPLETE) || ((zcl_attr_imageUpgradeStatus == IMAGE_UPGRADE_STATUS_WAITING_TO_UPGRADE) && (pUpgradeEndRsp->upgradeTime != 0xFFFFFFFF)))
    {
