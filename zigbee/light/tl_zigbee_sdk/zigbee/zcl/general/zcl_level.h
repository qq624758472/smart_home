...
typedef struct {
    ...
    u16 remainingTime;        //!< the time remaining until the current command is complete 当前命令完成之前的剩余时间
#endif
#ifdef ZCL_ATTR_ONOFF_TRANS_TIME_ENABLE
    u16 onOffTransTime;       //!< the time taken to move to or from the target level 移动到目标级别或从目标级别移动所花费的时间
#endif
