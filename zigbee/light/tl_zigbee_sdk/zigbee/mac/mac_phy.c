...
_attribute_ram_code_ u8 rf_performCCA(void)
{
    ...
    { // Return if currently in TX state
        return PHY_CCA_BUSY;
    }
    else
    {
...
_attribute_ram_code_ __attribute__((optimize("-Os"))) void rf_rx_irq_handler(void)
{
     ...
     * rf_busyFlag&TX_BUSY表示在被调用的csma延迟回叫中断期间之前接收到的分组，
     *应该丢弃这个数据包
     */
