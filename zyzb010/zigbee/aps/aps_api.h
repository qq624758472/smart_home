...
typedef struct{
	...
	/** \ref Endian "[LE]" The identifier of the profile for which   此帧所针对的配置文件的标识符
...
typedef struct{
	...
	*/
	u8	dst_addr_mode;
	/** The target endpoint on the local entity to which the ASDU is directed. ASDU指向的本地实体上的目标端点*/
	u8	dst_ep;
	...
	*/
	...
	u16	srcMacAddr;//MAC address indicate the device send this packet    //表示发送此数据包设备的MAC地址
