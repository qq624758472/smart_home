 ...
 * @Version: 2.0
 * @Autor: ruog__
 * @Date: 2024-10-14 10:58:58
 * @LastEditors: ruog__
 * @LastEditTime: 2024-11-11 11:12:16
 */
/********************************************************************************************************
...
unsigned char clamp(int value)
{
    ...
    {
        return 255; // 如果大于255，返回255
    }
    else
    {
        return (unsigned char)value; // 否则直接返回值
    }
}

// 计算调整后的RGB值
void adjust_brightness(unsigned char *R, unsigned char *G, unsigned char *B, unsigned char L)
{
    // 计算亮度的比例（0到255之间的比例）
    float brightness_factor = (float)L / 255.0;

    // 计算新的RGB值，确保每个通道的值在0到255之间
    *R = clamp(*R * brightness_factor);
    *G = clamp(*G * brightness_factor);
    *B = clamp(*B * brightness_factor);
}
void adjust_brightness_cw(unsigned char *C, unsigned char *W, unsigned char L)
{
    // 计算亮度的比例（0到255之间的比例）
    float brightness_factor = (float)L / 255.0;

    // 计算新的RGB值，确保每个通道的值在0到255之间
    *C = clamp(*C * brightness_factor);
    *W = clamp(*W * brightness_factor);
}
void hwLight_colorUpdate_HSV2RGB_MY(u8 R, u8 G, u8 B,u8 C, u8 W, u8 level)
{

#if 1
    LOG("RGB=(%d,%d,%d)level:%x\r\n", R, G, B, level);
    level = (level < 0x10) ? 0x10 : level;
    adjust_brightness(&R,&G,&B,level);
    adjust_brightness_cw(&C,&W,level);
    ...
    LOG("gammaCorrect RGB=(%d,%d,%d) CW=(%d,%d)", gammaCorrectR, gammaCorrectG, gammaCorrectB,gammaCorrectC,gammaCorrectW);
...
{
#ifdef ZCL_LIGHT_COLOR_CONTROL // 这个宏只要支持RGB或者CCT，这个宏就会被选上
