...
void zbdemo_bdbInitCb(u8 status, u8 joinedNetwork)
{
    if (status == BDB_INIT_STATUS_SUCCESS)
    {
         ...
         * */
        LOG("joinedNetwork:%d", joinedNetwork);
        if (joinedNetwork)
        {
            heartInterval = 1000;
...
void zbdemo_bdbCommissioningCb(u8 status, void *arg)
{
    printf("zbdemo_bdbCommissioningCb\r\n");
    if (status == BDB_COMMISSION_STA_SUCCESS)
    {
        ...
        if (!gLightCtx.bdbFindBindFlg)
        {
            ...
            // light_blink_start(2, 200, 200); //模组暂时没有led
...
{
    LOG("");
    // nv_flashWriteNew(1, NV_MODULE_APP, NV_ITEM_ADDRESS_FOR_BIND, 1, &factoryRst_powerCnt);
#if FIND_AND_BIND_SUPPORT
