...
#if AF_TEST_ENABLE
    /* A sample of AF data handler. 测试发送和接收数据的代码入口位置。   给GW发送和从GW接收数据的代码入口*/
    af_endpointRegister(SAMPLE_TEST_ENDPOINT, (af_simple_descriptor_t *)&sampleTestDesc, afTest_rx_handler, afTest_dataSendConfirm);
#endif

    /* Initialize or restore attributes, 初始化或恢复属性 this must before 'zcl_register()' */
...
extern void convertColorXYToRGB(int colorX, int colorY, float *R, float *G, float *B);
/*********************************************************************
 * @fn      user_init
 *
