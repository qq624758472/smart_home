...
{
    // led_off(LED1);
    led_on(LED1);//lsh  默认将输出信号置高电平， 低电平灯亮
}
...
void test_send_data()
{
    ...
    dstEpInfo.dstEp = 0x01;               // 所有节点的地址都是这个
    ...
    u8 seqNum = 0;
    af_dataSend(SAMPLE_MY_ENDPOINT, &dstEpInfo, ZCL_CLUSTER_TELINK_SDK_TEST_REQ, 32, buf, &seqNum);
...
void buttonKeepPressed(u8 btNum)
{
    if (btNum == VK_SW1)
    {
        g_switchAppCtx.state = APP_FACTORY_NEW_DOING; // lsh离网操作
        zb_factoryReset();
        LOG("switch to factory reset");
    }
}
