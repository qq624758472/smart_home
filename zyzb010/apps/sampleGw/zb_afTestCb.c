...
extern u16 clientShortAddr[64];
extern u8 clientNum;
/*统计上线的子节点短地址, 列表中没有的话，就加到列表中*/
void af_client_recv_request(u16 shortAddr)
{
    int i = 0;
    for (i = 0; i < clientNum; i++)
    {
        if (clientShortAddr[i] == shortAddr)
        {
            break;
        }
    }
    ...
    if (i == clientNum)
    {
        if(i >=64)
        {
            LOG("clientShortAddr list is full, can't add new client");
            return;
        }
        clientShortAddr[clientNum++] = clientShortAddr;
    }
}
/**********************************************************************
 * FUNCTIONS
...
static void afTest_testReqPrc(apsdeDataInd_t *pApsdeInd)
{
    ...
    af_client_recv_request(pApsdeInd->indInfo.src_short_addr);
...
void afTest_color_reqPrc(apsdeDataInd_t *pApsdeInd)
{
    epInfo_t dstEp;
    TL_SETSTRUCTCONTENT(dstEp, 0);
    ...
    u8 dataLen = sizeof(tiaoSeCMD);
    u8 *pBuf = (u8 *)ev_buf_allocate(dataLen);
    if (pBuf)
    {
        tiaoSeCMD *pData = pBuf;
...
void afTest_rx_handler(void *arg)
{
    ...
    case ZCL_CLUSTER_TELINK_SDK_TEST_CLEAR_REQ:
        ...
        break;
    case ZCL_CLUSTER_TELINK_SDK_TEST_REQ: // 上线请求
        g_afTest_rcvReqCnt++;
    ...
    // case ZCL_CLUSTER_TELINK_SDK_LIGHT_COLOR_REQ://调色指令。
        // afTest_color_reqPrc(pApsdeInd);
        // break;
    case ZCL_CLUSTER_TELINK_SDK_SWITCH_ONOFF_RSP://开关指令的回复指令。
        break;
    default:
        break;
    }
