...
{
    LOG("sampleGW_zclProcessIncomingMsg\n");
...
static void sampleGW_zclWriteRspCmd(zclIncoming_t *pInMsg)
{
    ...
    *pBuf++ = LO_UINT16(pInMsg->msg->indInfo.cluster_id);
    ...
    if (pInMsg->dataLen == 1)
    { // the case of successful writing of all attributes
 ...
 *
 * @brief   Handler for ZCL poll control request command.  ZCL polL控制请求命令的处理程序。
 *
 * @param   pApsdeInd
 * @param   seqNum
 *
